...
interface AgentPatientChatProps {
  selectedPatientId: string | null;
  selectedPatientName: string | null;
  onBack: () => void;
}

const AgentPatientChat: React.FC<AgentPatientChatProps> = ({
  selectedPatientId,
  selectedPatientName,
  onBack,
}) => {
  ...
  const messages = useMemo(() => {
    return selectedPatientId ? getChatMessages(selectedPatientId) : [];
  }, [selectedPatientId, getChatMessages, allMessages.length, allMessages]);
  ...
  const handleSend = async () => {
    if (!selectedPatientId || !messageText.trim() || !currentUserId) return;
    ...
    if (shouldSend) {
      sendMessage(selectedPatientId, messageToSend);
  ...
  if (!selectedPatientId) {
    return (
      <div 
        className="patient-chat-container flex items-center justify-center h-full bg-gray-50"
  ...
  return (
    <div 
      className="patient-chat-container flex flex-col h-full bg-white"
      style={{ position: 'relative', width: '100%', display: 'block', overflow: 'visible' }}
    >
      {/* Header */}
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center">
        ...
        <div className="flex-1">
          <h3 className="text-lg font-semibold text-gray-900">{selectedPatientName}</h3>
...
export default AgentPatientChat;
